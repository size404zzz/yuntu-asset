...
/* —— B 外源锚点 —— */
{
  ...
  if (manifest && fixture) {
    ...
    /* 2026-09-03：修 lvm.js 的 SETLIST off-by-one（每个数组字面量丢最后一条）
       之后，建议总数从 800+ 掉到 24。原先绝大多数「作者没写淡出」其实是淡出
       条目正好是数组末条被吞。样本只剩 imminent n=13 / distant n=8 / silent n=3，
       任何阈值都只是在拟合噪声，故按样本量显式跳过；锚点仍照常扫描并打印数字，
       等有新外源真值（如实机淡出序列）再重标。
       复核口径见 tools/audit-decode-completeness.mjs。 */
    if (sugN < 200) {
      console.log(`  skip 外源梯度锚点：解码修复后建议只剩 ${sugN} 条（旧基线 800+），`
          + `样本不足以钉分档`);
    } else {
