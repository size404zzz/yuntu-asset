...
## 检查器分层重构：舞台状态下放到各层，逐镜就地编辑（M35）

旧检查器第 0 区「舞台状态（截至本镜）」是只读罗列，改台上得跳回出处镜；
现在拆进各层分区（对白/背景/立绘/动画/音乐/CV/特效），每层上半展示
「台上现状」（`foldShotState` 折叠 0..本镜，延续项注明于第几镜起、可点击
跳转），控件直接改「本镜看到的状态」：

- **写入全部只落本镜**——可见度/槽位/明暗写本镜 `imgTween` 的 0 号状态帧
  （`upsertStateFrame`，并入已有 0 号帧、插在该元素首帧之前保持时序）；
  表情写本镜 `heroFace`；换图/换装/通讯框走本镜 delete+重注册（语料换装
  语义：回收后重建全新 item，`doc.structure` 打包注册+状态帧成一步撤销，
  原注册条目经 `foldShotState` 新增的 `src` 引用保真复制 fullScreen/order
  等字段）；BGM/底色是本镜调用，自本镜起按引擎语义延续到下一次改动。
  前后的镜数据一字不动（探针 28 项断言钉死）。
- **失效管线配套**：L2/L3 提交后按字段定点重建受影响分区
  （`refreshSections`：imgTween→背景+立绘、heroFace→立绘、bgColor→背景、
  audio→音乐），时间轴宿主原地保留（sel 不丢，自绘刷新）；时间轴指针
  交互期间挂起重建（`onBusy` 窗口：pointerdown 捕获开、pointerup/cancel
  冒泡到 document 关），松手后一次性补课。
- **动画区幽灵轨道**：`mountTimeline` 收 `stage`（在场元素清单），还没帧的
  元素给虚线轨道，点空白即为本镜插首帧（α/槽位取台上现状）——各层行的
  状态帧与时间轴关键帧是同一份数据。顺带修了 maxT 的展开优先级 bug
